...
public class MapScreen extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        for (Location loc : LocalDB.locations) {
            avgLat += loc.getLatitude(); //gonna center map on average of all pins so keep average data
            ...
            MarkerOptions pin = new MarkerOptions()
                    .position(pos)
                    .title(loc.getName())
                    .snippet(loc.getPhone() + " | " + loc.getType())
                    .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE));

            mMap.addMarker(pin);
        }

        LatLng avg = new LatLng(avgLat / cnt, avgLon / cnt);
